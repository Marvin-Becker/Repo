...
param(
		...
		[ValidateSet("Windows Boot Loader","Resume from Hibernate","Windows Memory Tester","Device options")]
		$pattern
)
		$id = $NULL
    bcdedit /enum all |
    ...
    ForEach-Object {
        $id += ($_.Context.PostContext[1] -replace '^identifier +')
        $id = $id.replace('}{','},{')
        $id = $id.split(',')[0]
    }
    return $id
}
GetIdentifier -pattern "Device options"
