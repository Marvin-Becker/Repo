<#
.SYNOPSIS
Trigger installation of a software bundle deployment in Tanium.

.DESCRIPTION
This endpoint starts an asynchronous Job to trigger a Package in Tanium. Only existing packages will run.

.PARAMETER servername
Target server for software bundle deployment. Validationpattern is '^[a-zA-Z0-9\-]{1,15}$'

.PARAMETER softwarePackagename
Name of the software bundle to trigger in Tanium. Validationpattern is '^[a-zA-Z0-9]{1,30}$'
...
if ($ServerCIValidationSucceded -eq $true) {
    $PossiblePackages = @{
        'serverStandard' = '[ASY-INSTALL-WIN] Server Deployment Softwarebundle'
    }

    $PackageTimeout = @{
        'serverStandard' = 18000
    }
...
if (
    ...
    try {
        $InvokeParams = @{
            'Script'              = 'Tanium\New-BundleDeployment.ps1'
