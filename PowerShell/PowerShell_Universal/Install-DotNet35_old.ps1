<#
.Synopsis
   .Net 3.5 installer for Windows Server 2012R2, 2016R1, 2019
   and 2022.
.DESCRIPTION
...
$WorkingDir = $Source + "\"
$DotNetPkg = "dotnet35packages.zip"
$global:rc = 0
$global:Info = @()
...
function Get-Package {
    $global:Info += "Infomessage: Get $DotNetPkg"
    try {
        Expand-Archive $WorkingDir$DotNetPkg -DestinationPath $WorkingDir -ErrorAction Stop
    } catch {
        throw "Errormessage: Extracting the archive failed"
    }
}
...
function Install-Dotnet {
    $global:Info += "Infomessage: install dotnet35"
    $DotNetSource = (Get-ChildItem -Path $WorkingDir\ | Where-Object -FilterScript { (Get-WinVer).toString() -match $_.Name }).FullName + "\sxs"
...
function Find-NewVersion {
    ...
    #We Explore the results from object the previous Searcher and we define title 
...
function Write-Log {
    ...
    $EventlogParams = @{
        ...
        Message     = ".Net35 was installed by Script - Result: $Errormessages"
